...
function insert_norebuild(index) {
    ...
    if (this.length === 0) {
        ...
        (_b = this._tasks).splice.apply(_b, __spreadArrays([index, 0], func));
        this.length = this._tasks.length;
    }
}
...
function rebuild_noawait() {
    if (this.length === 0)
        ...
        this.call = bake_collection_1.bakeCollection(this._tasks, this.argsNum);
}
function rebuild_await() {
    if (this.length === 0)
        ...
        this.call = bake_collection_1.bakeCollectionAwait(this._tasks, this.argsNum);
}
