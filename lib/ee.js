...
function listeners(event) {
    if (this.emit === emit)
        ...
        if (this.events[event] && this.onceEvents[event]) {
            return __spreadArrays(this.events[event].tasksAsArray(), (typeof this.onceEvents[event] === 'function' ? [this.onceEvents[event]] : this.onceEvents[event]));
        }
...
function eventNames() {
    ...
    if (this.emit === emit) {
        ...
        return __spreadArrays(keys, Array.from(this._symbolKeys)).filter(function (x) { return (x in _this.events) && _this.events[x] && _this.events[x].length; });
    }
    else {
        ...
        return __spreadArrays(keys, keysO, Array.from(this._symbolKeys).filter(function (x) { return (((x in _this.events) && _this.events[x] && _this.events[x].length) ||
